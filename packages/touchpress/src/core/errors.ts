...
export type ErrorInfo =
  ...
  | { readonly kind: 'ai-evaluate-unsupported' }
  | {
  ...
  switch (info.kind) {
    ...
    case 'ai-not-configured':
      return [
        ...
        "For act alone, use.evaluationModel takes a decision model instead, such as typeSafeAi.evaluationModel('jev-latest').",
      ].join('\n');
    case 'ai-evaluate-unsupported':
      return [
        "use.evaluationModel needs experimental_evaluate, which the installed 'ai' package does not export.",
